    ...
    fn next(&mut self) -> ParseResult<()> {
        loop {
            ...
            match event {
                ...
                XmlEvent::EndDocument | XmlEvent::CData(_) | XmlEvent::Characters(_) => {}
            }
    ...
    fn expected<T, E: ToString>(&self, expected: E) -> ParseResult<T> {
        ...
        Err(ParseError::UnexpectedXml {
            ...
            found: match self.cur {
                ...
                XmlEvent::Characters(ref data) | XmlEvent::CData(ref data) => {
                    Some(format!("\"{}\"", data))
                }
                _ => None,
            },
                    ...
                    "base64" => {
                        ...
                        let data = match self.cur.clone() {
                            XmlEvent::Characters(ref string) => {
                                ...
                                let stripped: Vec<_> = string
                                    .bytes()
                                    .filter(|b| !b" \n\t\r\x0b\x0c".contains(b))
                                    .collect();
                                base64::decode(&stripped)
                                    .map_err(|_| self.invalid_value("base64", string.to_string()))?
                            }
...
<methodResponse>
   <fault>
    ...
    fn parses_64bit_int() {
        assert_eq!(
            read_value("<value><i8>12345</i8></value>"),
            Ok(Value::Int64(12345))
        );
        assert_eq!(
            read_value("<value><i8>-100100100100</i8></value>"),
    ...
    fn parses_int_with_plus_sign() {
        ...
        assert_eq!(
            read_value("<value><int>+1234</int></value>"),
            Ok(Value::Int(1234))
        );
    }
    ...
    fn parses_base64() {
        assert_eq!(
            read_value("<value><base64>0J/QvtC10YXQsNC70Lgh</base64></value>"),
            Ok(Value::Base64("Поехали!".bytes().collect()))
        );

        assert_eq!(
            read_value("<value><base64> 0J/Qv tC10YXQ sNC70 Lgh  </base64></value>"),
            Ok(Value::Base64("Поехали!".bytes().collect()))
        );

        assert_eq!(
            read_value("<value><base64>\n0J/QvtC10\nYXQsNC7\n0Lgh\n</base64></value>"),
            Ok(Value::Base64("Поехали!".bytes().collect()))
        );
    }
    ...
    fn parses_empty_base64() {
        assert_eq!(
            read_value("<value><base64></base64></value>"),
            Ok(Value::Base64(Vec::new()))
        );
        assert_eq!(
            read_value("<value><base64/></value>"),
            Ok(Value::Base64(Vec::new()))
        );
    }
    ...
    fn rejects_attributes() {
        assert_err(read_value(
            r#"<value name="ble">\t  I'm a string!  </value>"#,
        ));
...
"##,
    ...
    fn duplicate_struct_member() {
        ...
        assert_eq!(
            ...
            Ok(Value::Struct(
                iter::once(("A".into(), "second".into())).collect()
            ))
        );
    }
}
